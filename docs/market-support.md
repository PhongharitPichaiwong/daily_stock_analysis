...
- 本次改动仅补齐 JP/KR 建议渲染与远端 stock-index market 白名单，不改动 provider/model/base URL 或运行时配置清理/迁移语义；未改造后端 bare-code 解析、历史/任务展示链路与模型路由边界，仍按既有行为运行，也未改动 `api/v1/endpoints/analysis.py`、`api/v1/endpoints/history.py`、`src/data/stock_index_loader.py`、`src/services/history_service.py` 等后端校验/展示路径。
