...
- `python -m pytest -q tests/test_task_queue_config_sync.py tests/test_system_config_service.py tests/test_config_manager.py tests/test_config_validate_structured.py tests/test_llm_channel_config.py tests/test_config_env_compat.py`（260 passed）
- Web 侧兼容性回归（本轮重跑）：
  - `cd apps/dsa-web && npm run test`（56 files passed，498 passed，500 tests total）。
