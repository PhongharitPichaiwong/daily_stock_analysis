...
兼容性核验结论：仅调整文本后处理映射与回测策略约束，未改动运行时模型/Provider/配置保存语义；如需回退，恢复旧的决策-回测映射与策略配置即可。
核验路径：本次变更仅在 `src/core/backtest_engine.py` 与 `tests/test_backtest_engine.py` 生效（含“先观望再买入/观望后买入”覆盖）；未在 `src/config.py`、`src/analyzer.py`、存储/持久化链路新增配置字段或清理逻辑。
