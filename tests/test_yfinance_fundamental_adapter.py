...
def _recent_quarterly_dividend_index() -> pd.DatetimeIndex:
    """Return four completed event dates that remain inside the rolling TTM window."""
    latest = pd.Timestamp.now(tz="America/New_York").normalize() - pd.Timedelta(days=1)
    return pd.DatetimeIndex(latest - pd.to_timedelta([270, 180, 90, 0], unit="D"))
...
class TestYfinanceFundamentalAdapter(unittest.TestCase):
    def test_populates_growth_earnings_dividend_boards_for_us_stock(self) -> None:
        ...
        dividends = pd.Series(
            [0.26, 0.26, 0.26, 0.27],
            index=_recent_quarterly_dividend_index(),
            name="Dividends",
        )
        ...
        self.assertEqual(div["events"][0]["ex_dividend_date"], dividends.index[-1].date().isoformat())
    ...
    def test_dividends_parsed_from_single_column_dataframe(self) -> None:
        ...
        idx = _recent_quarterly_dividend_index()
