...
- 回退方式：如新版 AlphaSift contract 运行异常，最小回退是将 `ALPHASIFT_ENABLED=false` 并重启后端；若必须回退适配层来源，可临时把 `ALPHASIFT_INSTALL_SPEC` pin 回旧 commit 并执行 `/api/v1/alphasift/install` 或重建运行环境，同时热点详情新版字段会降级为不可用或旧 provider fallback。
...
- 回滚边界（最小回滚）：
  - 用户级别先决：若仅需退出本次变更影响，可将 `ALPHASIFT_ENABLED=false` 重启；核心分析、日报报表与原有 LLM 调用链路不受该开关影响。
  - 依赖级别回退：仓库层面恢复到上一个版本的 `requirements.txt` / `src/config.py` / `.env.example` 并重建后端镜像/桌面后端产物（等价于 revert 本次 PR）；无须额外迁移脚本。
  - 安装入口回退：`/api/v1/alphasift/install` 支持的来源仅为 `DEFAULT_ALPHASIFT_INSTALL_SPEC`，若需用其他来源请先手动在环境中安装并重启。
